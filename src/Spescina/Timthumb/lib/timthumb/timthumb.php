<?php namespace Spescina\Timthumb\lib\timthumb;
/**
...
class tt {
	...
	public function __construct(){
		...
		if($this->isURL){
			...
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $this->localImageMTime . implode('&',$this->params) . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		}
	...
	protected function handleErrors(){
		if($this->haveErrors()){
			if(NOT_FOUND_IMAGE && $this->is404()){
				if($this->serveInternalImage(NOT_FOUND_IMAGE)){
					exit(0);
				} else {
	...
	protected function serveInternalImage($image = null){
		if($image){
    ...
    protected function setupParams() {
        $this->allowedSites = explode(',',env('ALLOWED_SITES', []));
    }
}
